...
function WorkExperience() {
    return (
        <div className='p-7 sm:pl-3 font-courier flex flex-col sm:flex-row sm:items-end gap-5 sm:gap-0'>
            ...
            <div className="border-none p-12 mx-auto relative text-wrap w-9/12">
                <div className="space-y-6 w-10/12 mx-auto">
                    {workExperience.map((exp) => (
                        <div key={exp.id} className="flex items-center space-x-8">
                            <div
                                className={`${exp.color} border ${exp.borderColor} w-12 h-12 rounded-md flex items-center justify-center text-xl font-bold p-10 text-black`}
                            >
                                {exp.id}
                            </div>
                            <div>
                ...
                <div className="absolute top-0 left-0 right-0 h-1 border-t border-black " />
                <div className="absolute bottom-0 left-0 right-0 h-1 border-b border-black " />
                <div className="absolute -top-10 bottom-5 left-10 border-l border-black h-[130%]" />
                <div className="absolute -top-10 bottom-5 right-10 border-r border-black h-[130%]" />
            </div>
        </div>
    )
}
